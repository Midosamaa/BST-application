...
void testPrint_order(){
   btree *root  =  createNode(15,0,NULL,NULL);
   insert(root,10,0);  insert(root,8,0);  insert(root,9,0); 
   insert(root,12,0);  insert(root,17,0);  insert(root,15,0);
   insert(root,23,0);  insert(root,20,0);  
   print_order(root);
   deleteTree(root); 
   printf("*** \x1B[32m Pass \e[0m : Test affichage en ordre ***\n");
}

void testPrint_root_fg_fd(){
   btree *root  =  createNode(13,0,NULL,NULL);
   insert(root,10,0);  insert(root,17,0);  insert(root,8,0); 
   insert(root,12,0);  insert(root,15,0);  insert(root,23,0);
   insert(root,9,0);  insert(root,20,0);  
   print_root_fg_fd(root);
   deleteTree(root); 
   printf("*** \x1B[32m Pass \e[0m : Test affichage root fgnode fdnode ***\n");
}
 ...
 void test_rotation(){
  ...
  t=rotationLeft(t);
  print_root_fg_fd(t);
  printf("*** \x1B[32m Pass \e[0m : Test Rotation left ***\n"); 
  t=rotationRight(t);
  print_root_fg_fd(t);
  deleteTree(t);
  printf("*** \x1B[32m Pass \e[0m : Test Rotation right ***\n"); 
 }
//  void testEquilibre ()
//  {
//    btree *t  =  createNode(15,0,NULL,NULL );
